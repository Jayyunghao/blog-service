...
import (
	...
	"Practice/go-programming-tour-book/blog-service/pkg/email"
	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				err := defailtMailer.SendMail(global.EmailSetting.To, fmt.Sprintf("异常抛出，发生时间:%d", time.Now().Unix()), fmt.Sprintf("错误信息: %v", err))
				if err != nil {
